module twiddle_interface #(
    ...
    output reg signed [NB_DATA-1:0]  o_data0_re, 
    output reg signed [NB_DATA-1:0]  o_data0_im,
    output reg signed [NB_DATA-1:0]  o_data1_re, 
    output reg signed [NB_DATA-1:0]  o_data1_im,
    output reg signed [NB_DATA-1:0]  o_data2_re, 
    output reg signed [NB_DATA-1:0]  o_data2_im,
    output reg signed [NB_DATA-1:0]  o_data3_re, 
    output reg signed [NB_DATA-1:0]  o_data3_im,
    output reg                       o_valid
);
...
reg [2*NB_DATA-1:0] mem_w1_fwd [0:7];
reg [2*NB_DATA-1:0] mem_w2_fwd [0:7];
reg [2*NB_DATA-1:0] mem_w3_fwd [0:7];
reg [2*NB_DATA-1:0] mem_w1_inv [0:7];
reg [2*NB_DATA-1:0] mem_w2_inv [0:7];
reg [2*NB_DATA-1:0] mem_w3_inv [0:7];

wire [2*NB_DATA-1:0] w1_selected, w2_selected, w3_selected;

initial begin
    $readmemb("twiddle_w1_fwd.mem", mem_w1_fwd);
    $readmemb("twiddle_w2_fwd.mem", mem_w2_fwd);
    $readmemb("twiddle_w3_fwd.mem", mem_w3_fwd);
    $readmemb("twiddle_w1_inv.mem", mem_w1_inv);
    $readmemb("twiddle_w2_inv.mem", mem_w2_inv);
    $readmemb("twiddle_w3_inv.mem", mem_w3_inv);
end
...
assign w1_selected = inv_q ? mem_w1_inv[cnt_q] : mem_w1_fwd[cnt_q];
assign w2_selected = inv_q ? mem_w2_inv[cnt_q] : mem_w2_fwd[cnt_q];
assign w3_selected = inv_q ? mem_w3_inv[cnt_q] : mem_w3_fwd[cnt_q];
